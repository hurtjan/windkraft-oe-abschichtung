...
| Als Nächstes | **W7.1 ist fertig und liegt auf `feat/w71-struktur-v4`** (`4e78ff7`, `806017d`), **nicht** auf `main`. Zwei Bedingungen für die Zusammenführung: der bandweise Vergleich ist sauber — **erfüllt** —, und das Dashboard hat das neue Manifest einmal wirklich gelesen — **offen**, die zweite Sitzung baut gerade Tiles. Danach zwei Dinge, in dieser Reihenfolge: **Punkt 65** (`validate` gegen `run1` bei ungleicher Bandzahl — schlafend, wacht beim nächsten Pixelwechsel auf) und dann die **Personenseilbahnen**, die der Nutzer am 09.09.2026 auf Gondeln und Sessellifte eingegrenzt hat. Die Reihenfolge ist nicht beliebig: Seilbahnen ändern Pixel und würden ohne Punkt 65 an einer sachfremden Stelle abbrechen. |
| **Referenz** | Seit W7.1 **`sha256 a905c056…5c9f`**, 152 669 124 Bytes, **44 Bänder**, Manifest 2.2.0. **Der Anker der Wellen 0 bis 6 ist damit bewusst abgelöst:** `fb57c41d…232c30` galt vom 07. bis 09.09.2026 und hat bewiesen, dass der Umbau nichts verändert — Welle 7 verändert absichtlich etwas, also musste er fallen. Der alte Wert bleibt hier stehen, die abgelöste Datei liegt als `abschichtung.tif.OLD` im Scratchpad. Davor: Bodensee-Korrektur (Punkt 33, `4bdef6ad…`) und Wegfall adressloser Großflächen (Punkt 34, `fb57c41d…`), beides Nutzerentscheidungen vom 08.09.2026. `run1` (`dc58b011…`) bleibt historischer Zeuge, **taugt aber seit W7.1 nicht mehr als maschinelle Vergleichsbasis** — 44 gegen 38 Bänder, Punkt 65. |
...
| Endprodukte | **Seit W7.1 sechs statt vier**: `out/abschichtung.tif` (152,7 MB, 44 Bänder), `out/abschichtung.bands.json`, `out/dashboard/` (**20 MB, 44 PNG-Overlays**), `out/gemeinden.geojson` (26,8 MB), **neu** `out/wka_bestand_punkte.geojson` (1595 Anlagen) und **neu** `out/LAYER.md`. `out/dashboard/` ist seit W6.7 die **Kartenansicht**: `index.html` mit Leaflet über OSM, ein PNG je Band, dazu `report.json` aus der Prüfstufe. **`out/LAYER.md` ist derzeit eine Kopie einer Handvorlage, kein erzeugtes Produkt** — ein Kopierziel legt sie ab, der Generator aus dem Manifest fehlt (Punkt 66). Bis W5.1 hat der Hauptbaum überhaupt nie etwas erzeugt — Punkt 48. |
...
| W7.1 | Der Produzent in einem Zug — Zuschnitt, Punkte-Export, sechs Bänder, Manifest 2.2.0 | **fertig** | 120 min | ≈70 min | **Bänder 1–37 bitidentisch, nur Band 38 geändert (−9390 Zellen, −3,40 %)** · Schnittmenge 38 ∧ 37 = **0** · 44 Bänder, Schema 2.2.0, `sha256 a905c056…5c9f` · Zählungen **exakt: 1595 / 807 / 788 / 183** · **235 + 3 Tests** · Dashboard 20 MB, 44 PNGs, alle sechs neuen Bänder mit Farbe · Maschinenzeit **8:27** · drei Bahnen parallel, **null Kollisionen** · `validate` bricht bei ungleicher Bandzahl ab — schlafend, Punkt 65 |
...
| # | Punkt | Fällig |
|---|---|---|
| 67 | **Alle 44 Bänder tragen `NoData = 0`, und das ist für dieses Raster wahrscheinlich falsch.** Bei den 34 Binärmasken ist die Null ein **gemessener Wert** — „Bedingung nicht erfüllt" —, keine fehlende Angabe; ein Betrachter, der `NoData` ernst nimmt, zeigt sie als Loch statt als Fläche. Bei den vier Unschärfebändern (0–100 %) wiegt es schwerer: dort verschwindet „0 %" aus den Daten. Möglich, dass es Absicht war, weil es Überlagerungen bequem transparent macht — belegt ist das nirgends, und in `docs/HANDOFF.md` steht dazu nichts. Gefunden am 09.09.2026 beim Nachmessen der Kopfdaten, **nicht** von Welle 7 verursacht: die abgelöste 38-Band-Fassung trägt denselben Wert. Zu klären, bevor jemand die Bänder außerhalb dieses Repos statistisch auswertet. | Sammelposten |
| 65 | **`pipeline/validate.py` bricht gegen `run1` ab, sobald die Bandzahl abweicht** — `measure_bands()` wirft ein `ValueError` (44 gegen 38), unbehandelt bis `sys.exit`, `make` meldet `Error 1`. **Derzeit schlafend:** `main()` steigt vorher mit `return 0` aus, wenn `out/abschichtung.tif` bitgleich zu `AKTUELLE_REFERENZ_SHA256` ist — genau der Fall seit W7.1. Der Abbruch **wacht beim nächsten Pixelwechsel auf**, oder sofort bei `--new-tif`. Dahinter steckt keine Reparatur, sondern eine Entscheidung: vergleicht das Register künftig nur die 38 überlappenden Bänder gegen `run1` und weist die sechs neuen als „ohne Gegenstück" aus — oder endet der Vergleich gegen `run1` mit Welle 7? `validate` hängt **nicht** am `all`-Ziel (`Makefile:166`), ein frischer Klon läuft also weiter durch. | **vor dem nächsten Paket, das Pixel ändert** |
| 64 | **Personenseilbahnen sollen nur Gondeln, Seilbahnen und Sessellifte sein, keine Schlepplifte.** Anforderung des Nutzers vom 09.09.2026. Ob und wo die Kette heute nach `aerialway`-Wert filtert, wird gerade erhoben — „nirgends gefiltert" ist als Ergebnis ebenso möglich wie eine unvollständige Liste. Ändert Pixel und damit den Referenz-Hash, gehört deshalb in ein eigenes Paket und **hinter Punkt 65**. | eigenes Paket |
| 63 | **Drei Eigenschaften des Punkte-Exports sind in allen 1595 Anlagen leer.** `pipeline/prep/osm.py` zieht mit `OSM_PBF_INCLUDE_TAGS["windpower"]` nur `power`, `generator:source`, `man_made`, `name` aus dem PBF; `operator`, `generator:output:electricity` und `start_date` werden nie extrahiert. Der Auswertungscode in `pipeline/export/wka_bestand.py` steht bereits und füllt sich von selbst, sobald die Tags mitgezogen werden. Gefunden in W7.1, nach Regel 4 gemeldet statt nebenbei geändert — die Datei gehört keiner der drei Bahnen. Das Dashboard fängt es über eine kuratierte Zweitquelle ab; der Produzent liefert weiter leer. Kostet einen OSM-Extraktionslauf. | eigenes Paket |
| 66 | **`out/LAYER.md` ist ein vertraglich zugesagtes Endprodukt, aber keine erzeugte Datei.** Ein Kopierziel (`make/export/layer_md.mk`) legt eine Handvorlage ab; `pipeline/export/layer_doc.py` als Generator aus dem Manifest wurde in W7.1 planmäßig abgebrochen, weil das 2.2.0-Manifest zu dem Zeitpunkt noch nicht existierte. Jetzt existiert es. Bis dahin gilt: **eine handgepflegte Datei, die 44 Bänder beschreibt und beim nächsten Bandwechsel still veraltet.** | Sammelposten |
